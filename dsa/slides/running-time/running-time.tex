...
\begin{frame}{Counting the Lines}
  ...
  \begin{alertblock}{Total}
    $\displaystyle 4(n-1) + 3\frac{(n-1)n}{2} + 1 = O(n^2)$
  \end{alertblock}
\end{frame}
...
\begin{frame}{Big-$O$}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      axis lines=left, tick style={draw=none},
      xtick={5.46}, xticklabels={$n_0$}, ytick=\empty,
      xlabel={$n$},
      ...
      \addlegendentry{$f(n)$}
      \draw[dashed, mGray] (axis cs:5.46,0) -- (axis cs:5.46,29.81);
    \end{axis}
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Example: $n^2 = O(1.5^n)$}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \draw[dashed, mGray] (axis cs:12.43,0) -- (axis cs:12.43,500);
    \end{axis}
  \end{tikzpicture}
  \end{center}
  \pause 
  \begin{center}
    From $n = 12.43$ onward, $n^2 \le 1.5^n$.
  \end{center}
\end{frame}
...
\begin{frame}{A Common Pitfall}
  ...
  \begin{alertblock}{What went wrong}
    ...
    Inside $\sum_{i=1}^{n} i$, the value $i$ ranges up to $n$, so it is \alert{not} a constant.
  \end{alertblock}
